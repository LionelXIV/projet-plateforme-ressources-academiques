...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Import Django-dependent things lazily to avoid import-time access to settings
        from django.contrib.auth import get_user_model

        USER_MODEL = get_user_model()
        Ressource = apps.get_model('core', 'Ressource')
        default_user = None
        ...
        for app_label, names in CANDIDATES.items():
            ...
            if not Model:
                self.stdout.write(self.style.WARNING(
                    f"Aucun modèle trouvé pour l'app '{app_label}' (candidates: {names})."))
                continue
                ...
                for obj in qs:
                    try:
                        ...
                        self.stderr.write(self.style.ERROR(
                            f"Erreur lors de la copie de {app_label} id={getattr(obj, 'pk', '?')}: {e}"))
                        self.stderr.write(traceback.format_exc())

            self.stdout.write(self.style.SUCCESS(
                f"Terminé {app_label}: copiés {summary[app_label]['copied']} / {summary[app_label]['objects']}"))
